...
public class Player : MonoBehaviour
{
    ...
    void Dodge()
    {
        if(jDown && (moveVec != Vector3.zero) && !isJump && !isDodge && !isSwap && !isDead)
        {
            dodgeVec = moveVec;//회피애니메이션발생중에 그 당시때의 진행방향벡터
            speed *= 2;
    ...
    void Interation()
    {
        if( nearObject != null && !isJump && !isDodge && !isDead)
        {
    ...
    private void OnTriggerEnter(Collider other)
    {
        if(other.tag == "Item")
        {
            ...
            if (other.GetComponent<Rigidbody>() != null) //원거리미사일 or 보스공격들의 경우에만 rigidBody가 있는 개체이며 이 경우에만 삭제
                Destroy(other.gameObject);
        }
    }
